...
{
    public:
        Renderer(float timestep, int numFrames, int numCells, float size, float radius, float visc, float diff, float rate, int numParticles);
        ~Renderer();
        void reset();
...
//        float getHz() { return m_hz; }
    ...
    private:
        ...
        FluidSolver m_fSolver;
        LevelSetSolver m_lsSolver;
